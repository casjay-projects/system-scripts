...
cp -r /root/.razor/ /var/amavis/ >"$LOGFILE" 2>>"$ERRORLOG"
razor-admin -d -create -home=/var/amavis/.razor/ >"$LOGFILE" 2>>"$ERRORLOG"
pyzor discover >"$LOGFILE" 2>>"$ERRORLOG"

sed -i 's|3|0|g' /root/.razor/razor-agent.conf >"$LOGFILE" 2>>"$ERRORLOG"
sed -i 's|DCCIFD_ENABLE=off|DCCIFD_ENABLE=on|g' /etc/dcc/dcc_conf >"$LOGFILE" 2>>"$ERRORLOG"
echo "razorhome = /var/amavis/.razor" >>/root/.razor/razor-agent.conf
...
cp -f /usr/libexec/dcc/rcDCC /etc/rc.d/init.d/DCC >"$LOGFILE" 2>>"$ERRORLOG"
ln -s /usr/libexec/dcc/cron-dccd /usr/bin/cron-dccd >"$LOGFILE" 2>>"$ERRORLOG"
chkconfig --add DCC >"$LOGFILE" 2>>"$ERRORLOG"

mkdir -p /var/amavis/.pyzor/ >"$LOGFILE" 2>>"$ERRORLOG"
cp /root/.pyzor/servers /var/amavis/.pyzor/ >"$LOGFILE" 2>>"$ERRORLOG"
