...
#
find $(cat $INCLUDEFILE) -type d -print > /tmp/include.tmp 2> /dev/null
mv -f /tmp/include.tmp $INCLUDEFILE.tmp
...
rm -Rfv $BACKUPDIR/$TWODAYBACKUPNAME >>$LOGFILE 2>>$ERRORLOG
sleep $SLEEPCMD ; tar cvpfz $BACKUPDIR/$BACKUPNAME --directory=/ --files-from=$INCLUDEFILE.tmp --exclude-from=$EXCLUDEFILE >>$LOGFILE 2>>$ERRORLOG
...
ENDTIME=$(date +"%r")
if [ ! -s $ERRORLOG ] ; then
if [ $SENDMAIL = "yes" ] && [ $EMAILbackupsystem = "yes" ]; then
echo -e "
$MAILHEADER\n
