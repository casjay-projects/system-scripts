...
if [ ! -d "$MYREPOBASE/$PROG" ]; then mkdir -p "$MYREPOBASE/$PROG"; fi
...
touch "$RPMPKGS"
/bin/rpm -qa --qf '%{name}\n' | grep -v gpg-pubkey | sort -u >"$BASEDIR/$PROG/$PROG.txt"
cp -f "$BASEDIR/$PROG/$PROG.txt" "$BASEDIR/$PROG/pkgs.txt" >>"$LOGFILE" 2>>"$ERRORLOG"
cp -f "$BASEDIR/$PROG/$PROG.txt" "$RPMPKGS" >>"$LOGFILE" 2>>"$ERRORLOG"
cp -f "$BASEDIR/$PROG/$PROG.txt" "$MYREPOBASE/$PROG/pkgs-$HOSTSHORT.txt" >>"$LOGFILE" 2>>"$ERRORLOG"
cat "$RPMPKGS" >>"$LOGFILE" 2>>"$ERRORLOG"

ENDDATE=$(date +"%m-%d-%Y")
ENDTIME=$(date +"%r")
if [ ! -s "$ERRORLOG" ]; then
  if [ $SENDMAIL = "yes" ] && [ "$EMAILrpmpkgs" = "yes" ]; then
    echo -e "
$MAILHEADER\n
