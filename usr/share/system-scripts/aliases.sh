...
cat /etc/aliases | awk '{print $1}' | sed 's/\://g' | sed '/^\#/d' | sed '/^$/d' | egrep -v "*mailman*|*admin*|*-bounces|*-confirm|*-join|*-leave|*-owner|*-request|*-subscribe|*-unsubscribe|*casjay*" >> $LOGFILE.tmp

for recipient in $(cat $LOGFILE.tmp)
do echo -e "This is an email test for $recipient\nThis message should have been sent to $recipient\n" | mail -r $MAILFROM -s "$MAILSUB $recipient alias" $recipient
done
