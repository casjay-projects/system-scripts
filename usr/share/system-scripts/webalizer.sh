...
if [ ! -f /usr/bin/webalizer ]; then
  echo -e "The webalizer program is not installed" >"$LOGFILE" 2>>"$ERRORLOG"
...
if [ -d "$WEBALIZERDIR" ]; then
  for conf in "$WEBALIZERDIR"/*.conf; do
    webalizer -c "$conf" >>"$LOGFILE" 2>>"$ERRORLOG" 2>&1
  done
fi
...
ENDTIME=$(date +"%r")
if [ ! -s "$ERRORLOG" ]; then
  if [ "$SENDMAIL" = "yes" ] && [ "$EMAILwebalizer" = "yes" ]; then
    echo -e "
$MAILHEADER\n
