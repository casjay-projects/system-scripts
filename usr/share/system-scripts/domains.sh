...
if [ ! -d "$MYREPOBASE" ]; then mkdir -p "$MYREPOBASE"; fi
if [ ! -d "$WWWDIR" ]; then mkdir -p "$WWWDIR"; fi
if [ ! -f "$HOSTEDDOMLIST" ]; then touch "$HOSTEDDOMLIST"; fi
...
if [ -f "$HTTPDCONF/httpd.conf" ] || [ -f "$HOSTEDDOMLIST" ]; then
  echo -e "Last updated on $STARTDATE at $STARTTIME\n" >"$HOSTEDDOMLIST"
  cat "$HTTPDCONF/httpd.conf" "$HTTPDCONF/vhosts.d"/*.conf | grep "ServerName" | awk '{print $2}' | grep -v -E "admin.*|myserverdomainname|ServerName|myserverdomainname|#" | sort -u >>"$HOSTEDDOMLIST"
  cat "$HOSTEDDOMLIST" >"$LOGFILE" 2>>"$ERRORLOG"
fi
...
ENDTIME=$(date +"%r")
if [ ! -s "$ERRORLOG" ]; then
  if [ "$SENDMAIL" = "yes" ] && [ "$EMAILdomains" = "yes" ]; then
    echo -e "
$MAILHEADER\n
